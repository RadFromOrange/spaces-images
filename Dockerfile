...
RUN sudo sed -i -e 's/\r$//' /usr/share/dev-scripts/install_OpenJDK_8_on_Ubuntu_focal.sh
...
RUN sudo sed -i -e 's/\r$//' /usr/share/dev-scripts/install_IDEA.sh
...
RUN sudo sed -i -e 's/\r$//' /usr/share/dev-scripts/create_IDEA_desktop_shortcut.sh
...
RUN sudo sed -i -e 's/\r$//' /usr/share/dev-scripts/configure_IDEA_to_autostart.sh
